...
Begin
	...
	select  @date0= convert(varchar(10),@date,120)   --今日
	select  @date1= convert(varchar(10),@date-1,120)  --昨日
  ...
  select *  into #tmp_Get_BossTotal from (
	 ...
	 from orders where 制单人 in (select * from @Wsc )--(select 编号 from pub_user where 姓名 ='公众号')
		...
		left join 
			( select  sum(case convert(datetime,convert(varchar(10),建立时间,120),120)  when @date0 then 1 else 0 end) as ND_count --今日新增会员
			  ...
			  from 会员 where  convert(datetime,convert(varchar(10),建立时间,120),120)  between @date1 and @date0 
			) b on 1=1
		left join
			( select  sum(case 日期 when @date0 then 金额合计 else 0 end) as ND_ReCharge  --今日充值
			  ...
			  from 充值表 where 制单人 in (select  编号 from pub_user where 姓名 ='公众号')
					And 日期  between @date1 and @date0
...
begin
	...
	select * 
	 from 会员 
		 where 状态=1 and 是否充值=0 and 截止日期>= @NowDate 
		       and  dbo.Fun_Birthday( 生日,@NowDate) 
