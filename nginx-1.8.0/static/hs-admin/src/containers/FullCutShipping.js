...
class FullCutShipping extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
      err: null,
  ...
  componentDidMount() {
    req.get('/uclee-backend-web/fullCutShipping').end((err, res) => {
      if (err) {
        return err
      }
        ...
        <div className="full-cut-shipping">
          ...
          <form onSubmit={this._submit} className="form-horizontal">
            <div className="form-group ">
              <label className="control-label col-md-3">设置满减运费：</label>
  ...
  _submit = e => {
    ...
    req.post('/uclee-backend-web/fullCutShippingHandler').send(data).end((err, res) => {
      ...
      if (res.text) {
        window.location = '/fullCutShipping'
      }
    })
  }
}

export default FullCutShipping
